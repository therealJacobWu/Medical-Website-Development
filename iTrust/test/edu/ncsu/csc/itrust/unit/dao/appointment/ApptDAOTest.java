...
public class ApptDAOTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        a2Date = new Timestamp(dateMillis + 1000 * 60 * 15);
        a3Date = new Timestamp(dateMillis + 1000 * 60 * 45);
    ...
    public void testGetAllAppointmentsWithinRange() throws Exception {
        List<ApptBean> appts = apptDAO.getAllAppointmentsWithinRange(beforeA1Date, a2Date);
        assertEquals(0, appts.size());

        apptDAO.scheduleAppt(a1);

        appts = apptDAO.getAllAppointmentsWithinRange(beforeA1Date, a1Date);
        ...
        appts = apptDAO.getAllAppointmentsWithinRange(beforeA1Date, a1Date);
        assertEquals(1, appts.size());
        appts = apptDAO.getAllAppointmentsWithinRange(beforeA1Date, a2Date);
        assertEquals(2, appts.size());
        apptDAO.scheduleAppt(a3);
        appts = apptDAO.getAllAppointmentsWithinRange(a2Date, a3Date);
        assertEquals(1, appts.size());
        ApptBean a3new = appts.get(0);
        assertEquals(a3new.getDate(), a3.getDate());
    }
